                ...
                HStack(spacing: 12) {
                    HStack(spacing: 4) {
                        Image(systemName: "heart.fill")
                            .font(.caption)
                        Text("\(socialService.getLikeCount(for: course.id))")
                            .font(.caption)
                    }
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(String(format: "%.1f", socialService.getRating(for: course.id)))
                            .font(.caption)
                    }
                }
            }
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        // Start Button
                        Button(action: {
                            // TODO: Navigate to course playback
                            dismiss()
                        }) {
                            Text("Start Course")
                                .font(.headline)
            ...
            VStack {
                HStack(spacing: 4) {
                    ForEach(1...5, id: \.self) { star in
                        Image(systemName: star <= (socialService.getUserRating(for: course.id) ?? 0) ? "star.fill" : "star")
                            .font(.title3)
                            .foregroundColor(.yellow)
                            .onTapGesture {
                                Task {
                ...
                Text(String(format: "%.1f (\(socialService.getLikeCount(for: course.id)))", socialService.getRating(for: course.id)))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
