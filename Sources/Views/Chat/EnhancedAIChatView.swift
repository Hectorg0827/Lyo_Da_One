...
struct EnhancedAIChatView: View {
    ...
    @State private var playingCinematic: A2UICinematic?
    ...
    var body: some View {
        NavigationStack {
            ...
            .fullScreenCover(item: Binding(
                get: { playingCinematic },
                set: { playingCinematic = $0 }
            )) { cinematicData in
                CinematicView(
                    data: cinematicData, 
                    onPlay: {
                        // In future: Start lesson flow
                        print("Playing cinematic lesson: \(cinematicData.title)")
                    },
                    onDismiss: {
                        playingCinematic = nil
                    }
                )
            }
        }
                ...
                LazyVStack(spacing: 16) {
                    ForEach(viewModel.messages) { message in
                        EnhancedMessageBubble(
                            ...
                            onCinematicPlay: { cinematicData in
                                playingCinematic = cinematicData
                            }
                        )
                        .id(message.id)
                    }
...
class AIChatViewModel: ObservableObject {
    ...
    func sendMessage(content: String, mode: AIChatMode, attachmentIds: [String]?) async {
        ...
        // ✅ TEST COMMAND: Test A2UI Component Rendering
        if content.lowercased() == "/testa2ui" {
            print("🧪 TEST: Simulating A2UI response...")
            let a2uiJSON = """
            {
                "type": "a2ui_component",
                "props": {
                        ...
                        "children": [
                            {
                                "type": "text",
                                "props": {
                                    "text": "This card was sent as a raw JSON payload and parsed by the client.",
                                    "foreground_color": "#006064"
                                }
                            },
                             {
                                "type": "button",
            ...
            """
            // Simulate AI response with this payload
            // We wrap it in markdown code block to test extraction too, or send raw
            let rawResponse = "Here is a dynamic component:\n\n```json\n\(a2uiJSON)\n```"
            
            // Manually process it through the pipeline
            let parseResult = A2UIParser.parse(rawResponse)
