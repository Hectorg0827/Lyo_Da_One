...
struct CommunityMapView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
                ...
                ForEach(viewModel.mapPins) { pin in
                    Annotation("", coordinate: pin.coordinate) {
                        MapPinView(pin: pin) {
                            ...
                            // Auto-expand dock to show details or context
                            withAnimation {
                                isDockExpanded = true
                            }
                        }
                        .scaleEffect(viewModel.selectedPin?.id == pin.id ? 1.2 : 1.0)
                        .animation(.spring(), value: viewModel.selectedPin?.id)
                    }
                }
            }
            .ignoresSafeArea()
            .onTapGesture {
                // Deselect if tapping map empty space
                withAnimation {
                    viewModel.selectedPin = nil
                    isDockExpanded = false // Optional: collapse dock
                }
            }

            // 2. Top Controls (Glass Search Bar)
            VStack(spacing: 0) {
                HStack(spacing: 12) {
                    // Search
                    HStack {
                ...
                // Location Button (Floating below header)
                HStack {
                    Spacer()
                    Button(action: {
                        HapticManager.shared.selection()
            ...
            .allowsHitTesting(true) // Ensure search bar interacts

            // 3. Right-Side Navigation Dock
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    case .studyGroup(let group):
                        StudyGroupDetailView(group: group, viewModel: viewModel)
