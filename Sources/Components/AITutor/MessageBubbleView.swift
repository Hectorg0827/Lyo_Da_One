...
struct LyoMessageBubbleView: View {
    ...
    /// When these structured blocks are present on an assistant turn, we skip the plain
    /// `Text` bubble so the user does not see duplicated JSON or the same copy twice.
    private var hidesPrimaryAssistantCopy: Bool {
        guard !message.isFromUser else { return false }
        guard let types = message.contentTypes else { return false }
        return types.contains(where: { contentType in
            switch contentType {
            case .quiz, .quizDeck, .flashcards, .studyPlan, .courseRoadmap:
                return true
            default:
                return false
            }
            ...
            VStack(alignment: message.isFromUser ? .trailing : .leading, spacing: DesignTokens.Spacing.xs) {
                ...
                if shouldRenderPrimaryText {
                    if message.isFromUser {
                        ...
                        LyoRichContentRenderer(text: message.content, highlights: [])
                            .padding(.horizontal, DesignTokens.Spacing.md)
                            .padding(.vertical, DesignTokens.Spacing.sm)
                            .background(messageBackground)
                    }
                ...
                if !message.isFromUser {
                    structuredEducationWidgets
                        .padding(.top, hidesPrimaryAssistantCopy ? DesignTokens.Spacing.xxs : 0)
                }
    ...
    @ViewBuilder
    private var structuredEducationWidgets: some View {
        if let types = message.contentTypes {
            ForEach(Array(types.enumerated()), id: \.offset) { _, contentType in
                educationBlock(for: contentType)
            }
        }
    }

    @ViewBuilder
    private func educationBlock(for contentType: MessageContentType) -> some View {
        switch contentType {
        ...
        case .courseProposal(let payload):
            ChatInteractiveCardView(
                ...
                onRefine: {
                    onQuickChipTap?(
                        "I want to refine the course '\(payload.title)' on \(payload.topic). Please offer options to adjust the difficulty, duration, or focus areas."
                    )
                },
                onSave: {
                    UIStackStore.shared.upsertCourse(
...
struct PremiumActionPillButton: View {
    ...
    private func iconForAction(_ type: MessageAction.ActionType) -> String {
        ...
        case .createCourseA2A: return "sparkles"
